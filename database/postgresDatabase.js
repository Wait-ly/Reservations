...
const getAllReservations = (id, callback) => {
  pool.connect()
    .then((client) => {
      client.query(`SELECT * FROM reservations INNER JOIN restaurants ON reservations.restaurant_id = restaurants.id INNER JOIN tables on reservations.restaurant_id = tables.restaurant_id WHERE reservations.restaurant_id = ${parseInt(id.slice(1))};`, (err, res) => {
        // const dictionary = {
